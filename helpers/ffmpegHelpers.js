...
exports.cropVideo = function(data) {
  var promise = new Promise(function(resolve, reject) {
    shell.exec(
      `ffmpeg -y -i ${data.video}.jpg -vf  "crop=1920:1080:0:0" ${
        data.video
      }_crop.jpg`,
      function(code, stdout, stderr) {
        let newData = {
...
exports.margeVideoAudio = function(data) {
  ...
  var promise = new Promise(function(resolve, reject) {
    shell.exec(
      `ffmpeg -y -loop 1 -i ${data.video}.jpg -i "${
        data.logo
      }" -filter_complex "overlay=(main_w-overlay_w)/2:(main_h-overlay_h)/2" -i "${
        data.audio
      }" -shortest -c:v libx264 -c:a copy ./dist/output/"${filename}".mkv`,
      function(code, stdout, stderr) {
        io.emit("makeVideo", {
          data: {
            code: code,
